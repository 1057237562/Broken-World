...
public class ElectrolyzerBlock extends ConsumerBlock {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (!world.isClient)
            return (world1, pos, state1, blockEntity) -> ((ElectrolyzerBlockEntity) blockEntity).tick(world1, pos,
                    state1, (ElectrolyzerBlockEntity) blockEntity);
        return null;
    }
