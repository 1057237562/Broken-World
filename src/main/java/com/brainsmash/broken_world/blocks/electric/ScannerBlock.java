...
import com.brainsmash.broken_world.blocks.entity.electric.ScannerEntity;
...
public class ScannerBlock extends ConsumerBlock {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if(!world.isClient)
            return (world1, pos, state1, blockEntity) -> ((ScannerEntity) blockEntity).tick(world1, pos, state1, (ScannerEntity) blockEntity);
        return null;
    }
}
