...
public class ColliderMultiBlock extends Multiblock {
    ...
    public boolean onDisassemble(boolean forced) {
        ...
        if (getWorld().getBlockEntity(pos) instanceof ColliderControllerBlockEntity entity) entity.onCoilBreak();
        return super.onDisassemble(forced);
    }

    public static void register() {
        MultiblockLib.INSTANCE.registerMultiblock(ID, ColliderMultiBlock::new,
                MultiblockPatternKeyBuilder.start().where('A', CachedBlockPosition.matchesBlockState(
                        state -> state.getBlock() == BlockRegister.get(BlockRegistry.COLLIDER_COIL))).where('B',
                        CachedBlockPosition.matchesBlockState(state -> state.getBlock() == BlockRegister.get(
                                BlockRegistry.COLLIDER_CONTROLLER))).build());
    }
