...
public class BWDensityFunctionTypes {
    ...
    public record Crater(DensityFunction center, double threshold, int searchRadius,
                         DensityFunction radius) implements DensityFunction.Base {
        ...
        public static final MapCodec<Crater> CRATER_CODEC = RecordCodecBuilder.mapCodec(
                (instance) -> instance.group(DensityFunction.FUNCTION_CODEC.fieldOf("center").forGetter(Crater::center),
                        ...
                        DensityFunction.FUNCTION_CODEC.fieldOf("radius").forGetter(Crater::radius)).apply(instance,
                        Crater::new));
        ...
        private static double height(double r, double R) {
            ...
            r *= EFFECTIVE_RANGE / R;
            return (1 - Math.pow(r / 2, 4)) * Math.exp(-25 * r * r / 49) + 5 * Math.exp(
                    -MathHelper.square((r - 15) / 3)) - 2 * Math.exp(-MathHelper.square((r - 5) / 5));
        }

        private static double weight(double r, double R) {
            r *= EFFECTIVE_RANGE / R;
            if (r <= TRANSITION_BORDER) return 1;
            r -= TRANSITION_BORDER;
            r /= 3;
            return Math.exp(-r * r);
        }
    ...
    public record ClampedGradient(DensityFunction input, double fromInput, double toInput, double fromValue,
        ...
        public static final MapCodec<ClampedGradient> CLAMPED_GRADIENT_CODEC = RecordCodecBuilder.mapCodec(
                (instance) -> instance.group(
                        ...
                        CONSTANT_DOUBLE_RANGE.fieldOf("to_value").forGetter(ClampedGradient::toValue)).apply(instance,
                        ClampedGradient::new));
        ...
        public double sample(NoisePos pos) {
            ...
            if (in <= fromInput) return fromValue;
            if (in >= toInput) return toValue;
    ...
    public record DiscretePoints(int gridWidth, int gridPadding, double minRadius, double maxRadius,
                                 DensityFunction whenPositive) implements DensityFunction.Base {
        ...
        public static final MapCodec<DiscretePoints> VOLCANO_CODEC = RecordCodecBuilder.mapCodec(
                (instance) -> instance.group(
                        ...
                        DensityFunction.FUNCTION_CODEC.fieldOf("when_positive").forGetter(
                                DiscretePoints::whenPositive)).apply(instance, DiscretePoints::new));
        ...
        public DensityFunction apply(DensityFunctionVisitor visitor) {
            return visitor.apply(
                    new DiscretePoints(gridWidth, gridPadding, minRadius, maxRadius, whenPositive.apply(visitor)));
        }
        ...
        public double sample(NoisePos pos) {
            ...
            if (whenPositive.sample(new BWNoisePos(x, 0, z)) <= 0) return -1;
    ...
    public record Debug(String tag, boolean enabled, DensityFunction argument) implements DensityFunction.Base {
        public Debug(String tag, boolean enabled, DensityFunction argument) {
            ...
            if (enabled) Debug.TAGS.put(tag, this);
        }
        ...
        public static final MapCodec<Debug> DEBUG_CODEC = RecordCodecBuilder.mapCodec(
                (instance) -> instance.group(STRING_CODEC.fieldOf("tag").forGetter(Debug::tag),
                        BOOL_CODEC.fieldOf("enabled").forGetter(Debug::enabled),
                        DensityFunction.FUNCTION_CODEC.fieldOf("argument").forGetter(Debug::argument)).apply(instance,
                        Debug::new));
        ...
        public double sample(NoisePos pos) {
            ...
            if (enabled) {
                int nop;
                if (x % 10000 == 0 && Math.abs(y - 150) <= 10 && z % 10000 == 0) nop = 42;
            }
    ...
    public record Flat(DensityFunction argument) implements DensityFunction.Base {
        public static final MapCodec<Flat> CODEC = RecordCodecBuilder.mapCodec((instance) -> instance.group(
                DensityFunction.FUNCTION_CODEC.fieldOf("argument").forGetter(Flat::argument)).apply(instance,
                Flat::new));
