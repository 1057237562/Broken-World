...
public class WColliderStatus extends WWidget {
    ...
    public void paint(MatrixStack matrices, int x, int y, int mouseX, int mouseY) {
        ...
        Vec2i[] directions = {Vec2i.NEGATIVE_Y, Vec2i.POSITIVE_X, Vec2i.POSITIVE_Y, Vec2i.NEGATIVE_X};
        Vec2i.Mutable p = new Vec2i.Mutable(x + gridWidth / 2 - l / 2 * w - w / 2, y + gridWidth / 2 + l / 2 * w - w / 2);
        for (int i = 0; i < 4; i++) {
            ScreenDrawing.texturedRect(
                    matrices,
                    p.getX(), p.getY(), w, w,
                    CORNER_ICONS,
                    i * 0.25f, 0f, i * 0.25f + 0.25f, 1f,
                    properties.get(propIndex + i * (l-1))
            );
            for (int j = 0; j < l - 2; j++) {
                ...
                ScreenDrawing.texturedRect(
                        ...
                        p.getX(), p.getY(), w, w,
                        SIDE_ICONS,
                        i % 2 * 0.5f, 0f, i % 2 * 0.5f + 0.5f, 1f,
                        properties.get(propIndex + i * (l-1) + 1 + j)
                );
            }
            p.move(directions[i].mul(w));
        }
