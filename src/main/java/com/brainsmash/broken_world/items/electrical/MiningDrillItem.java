...
public class MiningDrillItem extends BatteryHolderItem {
    protected final float miningSpeed;
    private final float attackDamage;
    private final ToolMaterial material;
    private final Multimap<EntityAttribute, EntityAttributeModifier> attributeModifiers;

    public MiningDrillItem(float attackDamage, float attackSpeed, ToolMaterial material, Settings settings) {
    ...
    public float getMiningSpeedMultiplier(ItemStack stack, BlockState state) {
        return getEnergy(stack) > 0 && (state.isIn(BlockTags.PICKAXE_MINEABLE) || state.isIn(BlockTags.SHOVEL_MINEABLE)) ? this.miningSpeed : 1.0f;
    }
